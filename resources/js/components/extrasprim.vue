    ...
    <div class="container-fluid">
        ...
        <div class="content-header" v-if="$gate.isAdmin()">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">{{ $trans[lang+'.main']['primsistemi'] }}</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="/dashboard">{{ $trans[lang+'.main']['dashboard'] }}</a></li>
                            <li class="breadcrumb-item "><a :href="'/'+pagelink">{{ $trans[lang+'.main']['primsistemi'] }}</a></li>
                            <li class="breadcrumb-item active">{{ $trans[lang+'.main']['prim'] }}</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header bg-success">
                        <h3 class="card-title">{{ $trans[lang+'.main']['prim'] }}</h3>
                    ...
                    <div class="card-body table-responsive p-0">
                        <table class="table table-hover">
                            ...
                            <tr v-for="extras,index in extras.data" :key="extras.id">
                                <td>{{ index+1 }}</td>
                                <td>
                                    <img v-if="extras.picture" :src="'/img/'+extrasimgpath+'/thumbs/' + extras.picture" >
...
<script>
    ...
    export default {
        data() {
            return {
                pagelink: "primsistemipage",
                findextras:'findextrasprim',
                extrasapi:'extrasprim',
                extrasupapi:'extrasprimup',
                extrasdownapi:'extrasprimdown',
                extrasimgapi:'extrasprim_image',
                extrasimgpath:'prim',
        ...
        methods: {
            ...
            getResults(page = 1) {
                axios.get('/api/'+this.extrasapi+'?page=' + page)
                    .then(response => {
            ...
            updateExtras() {
                ...
                this.form.put('/api/'+this.extrasapi+'/'+this.form.id)
                    .then(() => {
                        $('#addNew').modal('hide')
                        swal.fire(
                            'Updated!',
            ...
            createExtras() {
                this.$Progress.start();
                this.form.post('/api/'+this.extrasapi)
                    .then(({ data }) => {
            ...
            deleteExtras(id) {
                ...
                swal.fire({
                    ...
                    if(result.value) {
                        // send ajax request
                        this.form.delete('/api/'+this.extrasapi+'/'+id).then(()=>{
            ...
            loadExtras() {
                axios.get('/api/'+this.extrasapi).then(({ data})=> (this.extras=data));
            }
        },
        created() {
            Fire.$on('searching',() => {
                let query = this.$parent.search;
                axios.get('/api/'+this.findextras+'?q=' + query)
                    .then((data) => {
