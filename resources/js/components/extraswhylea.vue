    ...
    <div class="container-fluid">
        ...
        <div class="content-header" v-if="$gate.isAdmin()">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">{{ $trans[lang+'.main']['extraswhylea'] }}</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="/dashboard">{{ $trans[lang+'.main']['dashboard'] }}</a></li>
                            <li class="breadcrumb-item "><a href="/whylea">{{ $trans[lang+'.main']['whylea'] }}</a></li>
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" v-show="editmode" id="addNewModalLabel">{{ $trans[lang+'.extras']['updateinfowhylea'] }}</h5>
                        <h5 class="modal-title" v-show="!editmode" id="addNewModalLabel2">{{ $trans[lang+'.extras']['addinfowhylea'] }}</h5>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getResults(page = 1) {
                axios.get('/api/extraswhylea?page=' + page)
                    .then(response => {
            ...
            updateExtras() {
                ...
                this.form.put('/api/extraswhylea/'+this.form.id)
                    .then(() => {
                        $('#addNew').modal('hide')
                        swal.fire(
                            'Updated!',
            ...
            createExtras() {
                this.$Progress.start();
                this.form.post('/api/extraswhylea')
                    .then(({ data }) => {
            ...
            deleteExtras(id) {
                ...
                swal.fire({
                    ...
                    if(result.value) {
                        // send ajax request
                        this.form.delete('/api/extraswhylea/'+id).then(()=>{
            ...
            loadExtras() {
                axios.get('/api/extraswhylea').then(({ data})=> (this.extras=data));
            }
        },
        created() {
            Fire.$on('searching',() => {
                let query = this.$parent.search;
                axios.get('/api/findextraswhylea?q=' + query)
                    .then((data) => {
