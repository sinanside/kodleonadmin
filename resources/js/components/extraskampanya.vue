    ...
    <div class="container-fluid">
        ...
        <div class="content-header" v-if="$gate.isAdmin()">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">{{ $trans[lang+'.main']['kampanyalar'] }}</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="/dashboard">{{ $trans[lang+'.main']['dashboard'] }}</a></li>
                            <li class="breadcrumb-item "><a href="/kampanyalarpage">{{ $trans[lang+'.main']['pages'] }}</a></li>
                            <li class="breadcrumb-item active">{{ $trans[lang+'.main']['kampanyalar'] }}</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header bg-success">
                        <h3 class="card-title">{{ $trans[lang+'.main']['kampanyalar'] }}</h3>
                    ...
                    <div class="card-body table-responsive p-0">
                        <table class="table table-hover">
                            ...
                            <tr v-for="extras,index in extras.data" :key="extras.id">
                                <td>{{ index+1 }}</td>
                                <td>
                                    <img v-if="extras.picture" :src="'/img/kampanya/thumbs/' + extras.picture" >
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" v-show="editmode" id="addNewModalLabel">{{ $trans[lang+'.extras']['updateinfokampanya'] }}</h5>
                        <h5 class="modal-title" v-show="!editmode" id="addNewModalLabel2">{{ $trans[lang+'.extras']['addinfokampanya'] }}</h5>
                    ...
                    <form @submit.prevent="editmode ? updateExtras() : createExtras()">
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label class="col-sm-2 control-label">{{ $trans[lang+'.extras']['description'] }}:</label>

                                <div class="col-sm-12">
                                    <vue-editor :editorOptions="editorSettings"
                                                useCustomImageHandler
                                                @imageAdded="handleImageAdded"
                                                id="extraskampanya_editor"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getResults(page = 1) {
                axios.get('/api/extraskampanya?page=' + page)
                    .then(response => {
            ...
            updateExtras() {
                ...
                this.form.put('/api/extraskampanya/'+this.form.id)
                    .then(() => {
                        $('#addNew').modal('hide')
                        swal.fire(
                            'Updated!',
            ...
            createExtras() {
                this.$Progress.start();
                this.form.post('/api/extraskampanya')
                    .then(({ data }) => {
            ...
            deleteExtras(id) {
                ...
                swal.fire({
                    ...
                    if(result.value) {
                        // send ajax request
                        this.form.delete('/api/extraskampanya/'+id).then(()=>{
            ...
            loadExtras() {
                axios.get('/api/extraskampanya').then(({ data})=> (this.extras=data));
            }
        },
        created() {
            Fire.$on('searching',() => {
                let query = this.$parent.search;
                axios.get('/api/findextraskampanya?q=' + query)
                    .then((data) => {
